...
func FieldJSONInfo(f reflect.StructField) FieldTagInfo {
	return FieldInfo(f, "json")
}

func FieldInfo(f reflect.StructField, name string) FieldTagInfo {
	if !f.IsExported() {
		return FieldTagInfo{Omit: true}
	}
	info := FieldTagInfo{Name: f.Name}
	if tag, ok := f.Tag.Lookup(name); ok {
		info.NewTag(tag)
	}
	return info
}

func (info *FieldTagInfo) NewTag(tag string) {
